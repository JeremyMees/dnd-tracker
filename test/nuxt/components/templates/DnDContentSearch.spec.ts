...
let filterRef:
  ComputedRef<{ type: DndContentType; filters: DndContentFilters }> | undefined
...
vi.mock('~/queries/srd', () => ({
  useSrdListing: (
    filters: ComputedRef<{ type: DndContentType; filters: DndContentFilters }>,
  ) => {
    filterRef = filters

    return { data, status }
  },
  useSrdDocuments: () => ({ data: ref([]), status: ref('success') }),
}))
