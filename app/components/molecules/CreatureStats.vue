...
const resistanceTypes = Object.entries(resistanceTypeMap).map(
  ([key, label]) => ({ key: key as DndResistanceType, label }),
)

const speedEntries = computed(() =>
  props.creature.speed ? generateSpeedEntries(props.creature.speed) : [],
)
const sightEntries = computed(() =>
  props.creature.sight ? generateSightEntries(props.creature.sight) : [],
)
const skillEntries = computed(() =>
  props.creature.skillBonuses
    ? generateSkillEntries(props.creature.skillBonuses)
    : [],
)
const isResistant = computed(() =>
  props.creature.resistancesAndImmunities
    ? hasResistances(props.creature.resistancesAndImmunities)
    : false,
)
...
<template>
  <section class="flex flex-col gap-4">
    ...
    >
      ...
      <div class="flex gap-x-4 gap-y-1 flex-wrap">
        ...
        >
          ...
          <span>{{
            formatBonus(
              creature.savingThrows ? creature.savingThrows[label] : 0,
            )
          }}</span>
        </div>
      </div>
    ...
    <div v-if="isResistant" data-test-resistances>
      ...
      <div class="space-y-1">
        <div v-for="cat in resistanceTypes" :key="cat.key">
          <div
      ...
      <Card color="secondary" class="p-2" as="ul">
        ...
        >
          <div class="flex flex-wrap gap-x-4 items-center">
            <p class="font-bold">{{ trait.name }}:</p>
