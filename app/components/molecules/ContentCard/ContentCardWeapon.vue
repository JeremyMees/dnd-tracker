...
<template>
  <div class="text-sm">
    <p v-if="content.damageDice" class="mt-1 text-muted-foreground">
      <span class="font-bold text-foreground">Damage:</span>
      {{ content.damageDice }}
    ...
    <p v-if="content.isSimple !== undefined" class="mt-1 text-muted-foreground">
      <span class="font-bold text-foreground">Simple Weapon:</span>
      {{ content.isSimple ? 'Yes' : 'No' }}
    </p>
    <p
      v-if="content.isImprovised !== undefined"
      class="mt-1 text-muted-foreground"
    >
      <span class="font-bold text-foreground">Improvised:</span>
      {{ content.isImprovised ? 'Yes' : 'No' }}
    </p>
    ...
    <template v-if="isOpen">
      ...
      >
        ...
        >
          ...
          <div v-if="prop.detail" class="text-muted-foreground">
            {{ prop.detail }}
          </div>
        </li>
      </ul>
