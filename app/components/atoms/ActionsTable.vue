...
<template>
  <div v-if="actions.length" class="space-y-4">
    <div v-for="group in grouped" :key="group.actionType" class="space-y-2">
      <p class="head-6">
        {{ $t(`general.${group.actionType}`, 2) }}
      </p>
      <Card color="secondary" class="p-2" as="ul">
        <li
        ...
        >
          <div class="flex flex-wrap gap-x-3 gap-y-1 items-baseline">
            ...
            >
              ({{
                $t(
                  'components.actionsTable.cost',
                  { cost: action.legendaryActionCost },
          ...
          <template v-for="(attack, i) in action.attacks" :key="i">
            <div class="mt-2 flex flex-col gap-1">
              ...
              <div class="flex flex-wrap gap-x-4 gap-y-1 items-center">
                ...
                <div v-if="attack.toHitMod" class="flex gap-x-1 items-center">
                  <span class="font-semibold">To hit:</span>
                  <span class="text-muted-foreground"
                    >+{{ attack.toHitMod }}</span
                  >
                </div>
                ...
                >
                  <span class="font-semibold"
                    >{{ attack.reach !== undefined ? 'Reach' : 'Range' }}:</span
                  >
                  <span class="text-muted-foreground">{{
                    formatRange(attack)
                  }}</span>
              ...
              >
                ...
                >
                  ...
                  <span class="text-muted-foreground">
                    ...
                    <template v-if="attack.damageBonus">
                      + {{ attack.damageBonus }}</template
                    >
                    <template v-if="attack.damageType">
                      ({{ attack.damageType }})</template
                    >
                  </span>
                </div>
                ...
                >
                  ...
                  <span class="text-muted-foreground">
                    ...
                    <template v-if="attack.extraDamageBonus">
                      + {{ attack.extraDamageBonus }}</template
                    >
                    <template v-if="attack.extraDamageType">
                      ({{ attack.extraDamageType }})</template
                    >
                  </span>
                </div>

                <div v-if="attack.spellSave" class="flex gap-x-1 items-center">
                  <span class="font-semibold">Save:</span>
                  <span class="text-muted-foreground">
                    DC {{ attack.spellSave }}
                    <template v-if="attack.spellSaveType">
                      ({{ attack.spellSaveType }})</template
                    >
                  </span>
                </div>
              </div>
